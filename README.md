...
The first tab, `الملخص`, focuses on customer insights for exactly the exported visits (all or the selected date range): products ranked by average rating; customer knowledge; representative service, product presentation, invoices and offers; shelf/fridge placement; promotional-material availability percentages; and A–D customer/location classifications. Ratings show the average out of 5, that average divided by 5 as a percentage, and the response count. The rating percentage is not the percentage of customers (for example, 4/5 = 80%). There are no completion statistics, total-score columns or lengthy per-answer distributions. Blanks and invalid answers are excluded; numeric zero remains a valid rating. No valid responses display a dash rather than a misleading zero average or percentage. Percentages use valid Yes/No or A–D responses as their denominator, not all visits. Repeated visits to the same customer count separately.

The second tab, `ملاحظات الزبائن`, lists every written comment verbatim with customer name, visit date and topic. Empty notes are omitted; unnamed customers are labeled `غير مسمى`. Raw answers remain on `الزيارات`. Summary averages, counts and percentages reference that data with formulas and cached results. Product ranking and the separate comments are captured at export time; regenerate the workbook after changing visits.
